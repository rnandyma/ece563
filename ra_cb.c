...
{ 
    MPI_Bcast(c, (N/numP)*N, MPI_INT, x, MPI_COMM_WORLD);
    ...
    //MPI_Barrier(MPI_COMM_WORLD);
		...
		printf("the value of c(%d %d) is %d\n", i,j,c[i][j]);
                    ...
                    //cc[k][i] = sum;
                    //sum = 0;
	//MPI_Gather(&output[i][j], 1, MPI_INT, &d[i+(N/numP)*pid][j+(N/numP)*pid],1, MPI_INT, 0, MPI_COMM_WORLD);
            }
